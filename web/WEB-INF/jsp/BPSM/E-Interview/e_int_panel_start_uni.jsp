<%-- 
    Document   : e_int_panel_start_uni
    Created on : Mar 27, 2016, 4:28:22 AM
    Author     : Habib
--%>
...
String param_position_list[] = { ptj_is_refid, pph_refid };
...
String param_cand_list[] = { ptj_is_refid, pph_refid, ipl_refid, is_type_ptj, cand_status_accepted, pass_int_ptj };
...
<div class="row">
    <div class="well">
        ...
        <br/>
        <div class="row">
            <form method="post" action="process/bpsm/eInterview/e_int_go_interview_room_uni.jsp">
            ...
            for(int a = 0; a < data_cand_list.size(); a++)
            {
                ...
                <tr>
                    ...
                    {
                        %>
                        <td style="vertical-align: middle; text-align: center">DONE <a title="Edit" href="process.jsp?p=BPSM/E-Interview/e_int_interview_room_uni.jsp&is_refid=<%=ptj_is_refid %>&ipl_refid=<%=ipl_refid %>&pph_refid=<%=pph_refid %>&selected_irm_refid0=<%=data_cand_list.get(a).get(5) %>&selected_size=1" class="glyphicon glyphicon-pencil"></a></td>
                        <%
                    }
                    %>
                </tr>
        ...
        <div class="row">
            <form method="post" action="process/bpsm/eInterview/e_int_submit_to_chairman_process_uni.jsp">
                <input name="is_refid" value="<%=ptj_is_refid %>" type="hidden" />
                <input name="ipl_refid" value="<%=ipl_refid %>" type="hidden" />
                <%
                if(all_done)
                {
                    %>
                    <div class="col-sm-12"><button type="submit" class="btn btn-success form-control"><span style="color: white">Done</span></button></div>
                    <%
                }
                else if(!all_done)
                {
                    %>
                    <div class="col-sm-12"><button type="submit" class="btn btn-success form-control disabled"><span style="color: white">Done</span></button></div>
                    <%
                }
                %>
            </form>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a class="btn btn-warning form-control" href="process.jsp?p=BPSM/E-Interview/e_int_panel_start_uni_pos_list.jsp&is_refid=<%=ptj_is_refid %>&ipl_refid=<%=ipl_refid %>">OK</a>
            </div>
        </div>
        </form>
    </div>
