<%-- 
    Document   : e_pre_panel_start_uni
    Created on : Mar 25, 2016, 10:19:50 PM
    Author     : Habib
--%>
...
<div class="row">
    <div class="well">
        ...
        <br/>
        <div class="row">
            <form method="post" action="process/ptj/eInterview/e_pre_go_interview_room_uni.jsp">
            ...
            for(int a = 0; a < data_cand_list.size(); a++)
            {
                ...
                <tr>
                    ...
                    {
                        %>
                        <td style="vertical-align: middle; text-align: center">DONE <a title="Edit" href="process.jsp?p=PTJ/E-Interview/e_pre_interview_room_uni.jsp&is_refid=<%=ptj_is_refid %>&ipl_refid=<%=ipl_refid %>&pph_refid=<%=pph_refid %>&selected_irm_refid0=<%=data_cand_list.get(a).get(5) %>&selected_size=1" class="glyphicon glyphicon-pencil"></a></td>
                        <%
                    }
                    %>
                </tr>
        ...
        <div class="row">
            <form method="post" action="process/ptj/eInterview/e_pre_submit_to_chairman_process_uni.jsp">
                <input name="is_refid" value="<%=ptj_is_refid %>" type="hidden" />
                <input name="ipl_refid" value="<%=ipl_refid %>" type="hidden" />
                <%
                if(all_done)
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a class="btn btn-warning form-control" href="process.jsp?p=PTJ/E-Interview/e_pre_panel_start_uni_pos_list.jsp&is_refid=<%=ptj_is_refid %>&ipl_refid=<%=ipl_refid %>">OK</a>
            </div>
        </div>
        </form>
    </div>
