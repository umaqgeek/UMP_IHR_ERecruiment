...
<%
    if(!request.getParameter("accept_confirm").isEmpty() && !request.getParameter("pa_refid").isEmpty() && !request.getParameter("c_refid").isEmpty())
    {
        ...
        String sql_another_offer_pending = "SELECT PA.PA_REFID "
                + "FROM POS_APPLIED PA, CANDIDATE C "
                + "WHERE C.C_REFID = ? "
                + "AND PA.PA_REFID != ? "
                + "AND (PA.PA_STATUS != ? OR PA.PA_STATUS != ? OR PA.PA_STATUS != ? OR PA.PA_STATUS != ? )";
        String param_another_offer_pending[] = { sC_refid, sPa_refid, stat_accepted, stat_rejected, stat_sent, stat_activated };
        ...
        if(data.size() > 0)
        {
            ...
            {
                //if status OFFER PENDING, another offer will change to ANOTHER OFFER ACCEPTED
            ...
            {
                //if status OFFER SENT, another offer will change to OFFER REJECTED
                params3[0] = stat_rejected;
                params3[1] = sPa_refid;
                params3[2] = sC_refid;
            }
